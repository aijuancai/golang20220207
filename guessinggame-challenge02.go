...
func main() {
	...
	// second parameter is the default value (3)
	// the third is the description of the flag
	num := flag.Int("n", 3, "number of tries")
	...
	// create a for loop that begins at 1 and loops to count
	for try := 1; try <= count; try++ {
		...
		if try == count {
			// if the number of tries is equal to count, print game over and also the correct number
